#!/usr/bin/env python3
"""
Planilla tipo estudio: percepciones IIBB practicadas en **ventas** (facturas de cliente).

Columnas (misma idea que la planilla del contador):
- Nro.Inscripcion → `res.partner.l10n_ar_gross_income_number`
- Cuit → 11 dígitos desde `l10n_ar_vat` / `vat`
- Neto Gravado → base de la línea de impuesto (`tax_base_amount`), con signo en NC
- Fecha → `invoice_date` del comprobante (formato m/d/aaaa como en la muestra del estudio)
- Percepcion → importe de la línea de impuesto (signo alineado a NC vs factura)
- Nro.Percepcion → **solo dígitos** del número de comprobante fiscal (`l10n_latam_document_number`;
  si falta, se intenta extraer de `name`), en lugar del nombre del impuesto.

Origen de datos: misma lógica que `PERCEIIBB/generar_perceiibb_arca_master_dev.py`
(`account.move` publicado + `account.move.line` con `tax_line_id` en percepción IIBB).

Salida: `exportador-excel/out/percepciones_iibb_ventas_estudio_<desde>_a_<hasta>.xlsx`
"""
...
def _base_perc_signed(move_type: str, tax_base_amount: Any, balance: Any) -> tuple[Decimal, Decimal]:
    """Alinea base e importe al criterio de planilla (NC con montos negativos)."""
    base = _d2(tax_base_amount).quantize(DEC2, rounding=ROUND_HALF_UP)
    bal = _d2(balance)
    if bal == 0:
        bal = _d2(0)  # reservado si hiciera falta credit-debit
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    ap.add_argument(
        ...
        help="Columna Nro.Percepcion: solo dígitos del comprobante (defecto) o PV(4)+'-'+(8) tipo 0050-00000014.",
    )
    ...
    if not tax_perc:
        raise SystemExit(
            "No hay impuestos de percepción IIBB (l10n_ar_tax_type ~ perception o nombre PERC+IIBB/SIRCAR)."
        )
    ...
    if not line_ids:
        raise SystemExit("Hay facturas en el rango pero ninguna línea de impuesto de percepción IIBB.")
    ...
    stem = f"percepciones_iibb_ventas_estudio_{args.desde}_a_{args.hasta}"
    ...
    with tmp_csv.open("w", encoding="utf-8", newline="") as f:
        ...
        for l in lines:
            ...
            nro_fact_dig = _nro_factura_columna(inv, args.nro_factura_formato)
